...
*Figure 4*
```{r}
evening <- subset(afternoon, cap_metro$time_of_day=="Afternoon")
...
```{r}
kable(cor(wine_copy$pc1,wine_quant))
```
...
ggplot(wine_copy, aes(x = color, y = pc2,group=color)) +geom_boxplot() + xlab("Color") + ylab("Principal Component 1 Score") + ggtitle("Principal Component 2 vs. Color")
```

```{r}
wine_copy$pc2 <- wine_pca$x[,2]
ggplot(wine_copy, aes(x = quality, y = pc2,group=color)) +geom_point() + xlab("Quality") + ylab("Principal Component 1 Score") + ggtitle("Principal Component 2 vs. Color")
```
